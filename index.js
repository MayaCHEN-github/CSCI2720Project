...
class SingleLocation extends React.Component {
  ...
  fetchComments = async () => {
    try {
      const response = await fetch(`http://localhost:3000/api/venues/${this.props.locationData.venueId}`); 
      if (!response.ok) {
        console.error('Error fetching venues:', response.status);
        return;
      }
  ...
  handleAddComment = async () => {
    ...
    if (commentData.name) {
      try {
        const response = await fetch(`http://localhost:3000/api/venues/comment`, {
          method: 'POST', 
          headers: {
            'Content-Type': 'application/json',
          },
...
class StoredEventContent extends React.Component {
  ...
  deleteEvent = async (eventId) => {
    try {
      const response = await fetch(`http://localhost:3000/admin/event/${eventId}`, {
        method: 'DELETE'
      });
            ...
            <tbody>
              {UsersInfo.map(user => (
                <tr key={user.name}>
                  ...
                  <td>{user.type}</td>
                  <td>
                    <button className="btn btn-outline-primary action-button me-2" onClick={() => this.props.handleRead(user.name)}>
                    Read
                    </button>
                    <button className="btn btn-outline-danger delete-button me-2" onClick={() => this.props.handleDelete(user.name)}>
                      Delete
                    </button>
                    <button className="btn btn-outline-warning action-button me-2" onClick={() => this.props.handlUpdate(user.name)}>
                      Update
                    </button>
                  </td>
                </tr>
