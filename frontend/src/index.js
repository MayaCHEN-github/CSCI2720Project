...
class StoredEventContent extends React.Component {
  ...
  readEvent = (eventId) => {
    this.props.navigate('/read-event', { state: { eventId } });
    //console.log('index: ',eventId);
  }

  updateEvent = (eventId) => {
    this.props.navigate('/update-event', { state: { eventId } });
  }
  render() {
              ...
              {eventsInfo.map(event => (
                <tr key={event.eventId}>
                  ...
                  <td>
                    ...
                    <button className="btn btn-outline-warning action-button me-2" onClick={() => this.updateEvent(event.eventId)}>
                      Update
                    </button>
                  </td>
                </tr>
...
class User extends React.Component {
  ...
  deleteUser = async (name) => {
    try {
      ...
      if (response.status === 404) {
        ...
        console.error(`There is no user with name: ${name}`);
      } else if (response.status === 204) {
        // If the delete was successful, remove from the state
        this.setState(prevState => ({
  ...
  readUser = (name) => {
    this.props.navigate('/read-user', { state: { name } });
    //console.log('index: ',name);
  }

  updateUser = (name) => {
    this.props.navigate('/update-user', { state: { name } });
  }
  ...
  render() {
    ...
    return (
      <div className="stored-userss-container container ">
            ...
            <tbody>
              {UsersInfo.map(user => (
                <tr key={user.name}>
                  ...
                  <td>{user.type}</td>
                  <td>
                    <button className="btn btn-outline-primary action-button me-2" onClick={() => this.readUser(user.name)}>
                      Read
                    </button>
                    <button className="btn btn-outline-danger delete-button me-2" onClick={() => this.deleteUser(user.name)}>
                      Delete
                    </button>
                    <button className="btn btn-outline-warning action-button me-2" onClick={() => this.updateUser(user.name)}>
                      Update
                    </button>
                  </td>
                </tr>
        ...
        <button className="btn btn-outline-success create-button" onClick={this.handleCreateUser}>+ Create New User</button>
    ...
    return (
      <BrowserRouter>
        <Routes>
          ...
          <Route path="/admin-dashboard" element={<AdminDashboardWithNavigate />} />
