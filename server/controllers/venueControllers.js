...
const getVenues = async (req, res) => {
    ...
    //---------- get venues with different latitude & longitude
    // get venues with at least 3 events with latitude not zero
    const venuesAll = await Venue.find({ venueId: { $in: venuesListId }, latitude: { $ne: 0 } });
    // for each latitude remain only one venue
    for (let i = 0; i < venuesAll.length - 1; i++) {
        for (let j = i + 1; j < venuesAll.length; j++) {
            if (venuesAll[i].latitude === venuesAll[j].latitude) {
                venuesAll.splice(j, 1);
                j--;
            }
        }
    }
